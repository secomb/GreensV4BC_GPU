...
void analyzenet()
{
	...
	//extern float tlength,tlengthq,tlengthqhd;//added 8/09, 8/10
	extern float *diam,*lseg,*rseg,*ds,*ss,*axt,*ayt,*azt,*hd;// removed q,qq November 2016
...
/*
		...
		iseg = nodseg[1][inod];	//useful for identifying boundary flows - June 2013
		if(ista[iseg] == inod) printf("%i %f\n",inod,q[iseg]);
		else if(iend[iseg] == inod) printf("%i %f\n",inod,-q[iseg]);
		else printf("%i Error\n",inod);
*/
...
//start[k][iseg] = coordinates of starting point of segment i
	...
	for(iseg=1; iseg<=nseg; iseg++) if(segtyp[iseg] == 4 || segtyp[iseg] == 5){
		...
		//qq[iseg] = fabs(q[iseg]);//removed November 2016
		lseg[iseg] = 0.;
		for(k=1; k<=3; k++){
